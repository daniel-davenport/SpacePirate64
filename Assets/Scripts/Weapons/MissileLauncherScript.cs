...
public class MissileLauncherScript : MonoBehaviour
{
    ...
    weapon behavior:
    - Fires a missile with a long cooldown that automatically homes in on the first enemy it sees
    - Higher levels decrease cooldown, increase damage, and projectile speed
    ...
    public void FireWeapon(int slot, bool isChargedShot)
    {
        ...
        ProjectileInfo projInfo = firedShot.GetComponent<ProjectileInfo>();

        if (projInfo != null)
        {
            projInfo.explosionDamage = weaponInfo.explosionDamage;
            projInfo.explosionRadius = weaponInfo.explosionRadius;
        }

        Rigidbody rb = firedShot.GetComponent<Rigidbody>();


        // moving it forwards until it locks onto an enemy
        if (rb != null)
        {
